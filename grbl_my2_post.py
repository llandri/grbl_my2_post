...
SHOW_EDITOR = False                                # default show the resulting file dialog output in GUI
...
G0 X0.000 Y0.000
M30
'''                                                             # default postamble text will appear following the last operation milling
POSTAMBLE_DRILL = '''
M7
M73
G0 Z25.000
G0 X0.000 Y0.000
M30
'''                                                             # # default postamble text will appear following the last operation drilling
...
def processArguments(argstring):
    ...
    global PRECISION
    global PREAMBLE
    global POSTAMBLE_MILL
    global POSTAMBLE_DRILL
    global UNITS
    ...
    try:
        ...
        if args.postamble is not None:
            POSTAMBLE_MILL = args.postamble
        if args.postamble is not None:
            POSTAMBLE_DRILLILL = args.postamble
    ...
    if hasattr(pathobj, "Group"):    # We have a compound or project
        ...
        for c in pathobj.Path.Commands:
            ...
            for param in params:
                if param in c.Parameters:
                    if param == 'F':
                        ...
                        TOOL_NUMBER = int(c.Parameters['T'])
                        outstring.append(param + str(TOOL_NUMBER))
                    elif param in ['S']:
                        TOOL_SPEED = int(c.Parameters['S'])
                        outstring.append(param + str(TOOL_SPEED))
            ...
            # if command in ('G98', 'G99'):
                ...
                # out += "(Ничего не понять)"

            # if command in ('G90', 'G91'):
                # MOTION_MODE = command
                # out += "(Ничего не понять2)"
